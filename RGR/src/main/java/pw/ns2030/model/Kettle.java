...
public class Kettle extends Appliance {
    ...
    // Скорости уменьшены в 4 раза
    private static final double HEATING_RATE = 2.0;    // Нагрев: 20°C → 100°C за ~40 секунд
    private static final double COOLING_RATE = 0.5;    // Остывание: 100°C → 20°C за ~160 секунд
     ...
     */
    public Kettle(String id, String name, double power) {
        super(id, name, power);
        this.temperature = ROOM_TEMPERATURE;
    }

    /**
     * Конструктор с дефолтной мощностью 2000 Вт.
     */
    ...
    public void onPowerLoss() {
        if (state == PowerState.HEATING) {
            // Чайник сразу выключается, а не переходит в COOLING
            state = PowerState.OFF;
            System.out.println(name + ": Отключение питания - чайник выключен");
        }
    }
